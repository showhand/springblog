    ...
    $(document).ready(function () {
        ...
        $('.viewEntryAuditLink').click(function (event) {
            ...
            $("#auditList").jqGrid({
                                       ...
                                       datatype: 'json',
                                       mtype: 'GET',
                                       colNames: ['${labelLastModifiedDate}',
                                                  '${labelLastModifiedBy}', '${labelPostDate}',
                                                  '${labelSubject}', '${labelBody}',
...
<div id="blogEntry">
            ...
            <td>
                <c:if test="${not empty entry.attachments}">
                    <c:forEach items="${entry.attachments}" var="attachment">
                        <a href="/springblog/blogs/filedownload/entry/${attachment.id}"
    ...
    <shiro:hasRole name="ROLE_ADMIN">
        | <a href="javascript:void(0);" id="${entry.id}" class="viewEntryAuditLink">View Audit
        History</a>
    </shiro:hasRole>
...
<div id="entryAttachmentUpload" hidden="true">
    <h3>File Upload</h3>
    <form:form action="fileupload" modelAttribute="uploadItem" name="fileuploadForm" method="post"
                    ...
                    <td>
                        <c:if test="${not empty comment.attachments}">
                            <c:forEach items="${comment.attachments}" var="attachment">
                                <a href="/springblog/blogs/filedownload/comment/${attachment.id}"
                                   target="_blank">${attachment.fileName}</a>
                                <br/>
                            </c:forEach>
                        </c:if>
            ...
            <shiro:authenticated>
                <c:if test="${comment.postBy == pageContext.request.userPrincipal.name}">
                    ...
                    <a href="javascript:void(0);" id="${comment.id}"
                       class="commentAttachmentUpload">Upload Attachment</a>
                </c:if>
            </shiro:authenticated>
            <div id="commentAttachmentUpload_${comment.id}" hidden="true">
                <h3>File Upload</h3>
                <form:form action="fileupload" modelAttribute="uploadItem" name="fileuploadForm"
