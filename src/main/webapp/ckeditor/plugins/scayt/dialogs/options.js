...
CKEDITOR.dialog.add('scaytcheck', function (a) {
    var b = true, c, d = CKEDITOR.document, e = a.name, f = CKEDITOR.plugins.scayt
        .getUiTabs(a), g, h = [], i = 0, j = ['dic_create_' + e + ',dic_restore_' + e,
                                              'dic_rename_' + e + ',dic_delete_' + e], k = [
        'mixedCase', 'mixedWithDigits', 'allCaps', 'ignoreDomainNames'];

    function l() {
        if (typeof document.forms['optionsbar_' + e] != 'undefined') {
            return document
                .forms['optionsbar_' + e].options;
        }
        return[];
    };
    function m() {
        if (typeof document.forms['languagesbar_' + e] != 'undefined') {
            return document
                .forms['languagesbar_' + e].scayt_lang;
        }
        return[];
    };
    function n(z, A) {
    ...
    var o = a.lang.scayt, p = [
        {id: 'options', label: o.optionsTab, elements: [
            {type: 'html', id: 'options', html: '<form name="optionsbar_' + e
        ...
        {id: 'langs', label: o.languagesTab, elements: [
            {type: 'html', id: 'langs', html: '<form name="languagesbar_' + e
        ...
        {id: 'dictionaries', label: o.dictionariesTab, elements: [
            {type: 'html', style: '', id: 'dictionaries', html: '<form name="dictionarybar_' + e
        ...
        {id: 'about', label: o.aboutTab, elements: [
            {type: 'html', id: 'about', style: 'margin: 5px 5px;', html: '<div id="scayt_about_' + e
                + '"></div>'}
        ]}
    ...
    var s = function () {
        var z = this, A = z.data.scayt.getLangList(), B = ['dic_create', 'dic_delete', 'dic_rename',
                                                           'dic_restore'], C = [], D = [], E = k, F;
        if (i) {
            for (F = 0; F < B.length; F++) {
                C[F] = B[F] + '_' + e;
                d.getById(C[F]).setHtml('<span class="cke_dialog_ui_button">' + c['button_' + B[F]]
                                            + '</span>');
            }
        ...
        if (f[0] == 1) {
            for (F in E) {
                ...
                if ('undefined' != typeof I && 'undefined' != typeof c[G] && 'undefined' != typeof z
                    .options[E[F]]) {
                    I.setHtml(c[G]);
                    var J = I.getParent();
        ...
        var K = '<p><img src="' + window.scayt.getAboutInfo().logoURL + '" /></p>' + '<p>' + c
            .version + window.scayt.getAboutInfo().version.toString() + '</p>' + '<p>' + c
                    .about_throwt_copy + '</p>';
        ...
        var L = function (U, V) {
            ...
            var X = d.createElement('div'), Y = CKEDITOR.dom.element
                .createFromHtml('<input id="cke_option' + U + '" type="radio" ' + (z.sLang == U
                ? 'checked="checked"' : '') + ' value="' + U + '" name="scayt_lang" />');
            Y.on('click', function () {
                this.$.checked = true;
                z.chosed_lang = U;
            });
