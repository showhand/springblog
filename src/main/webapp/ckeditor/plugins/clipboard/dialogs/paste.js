...
CKEDITOR.dialog.add('paste', function (a) {
    ...
    return{title: b.title, minWidth: CKEDITOR.env.ie && CKEDITOR.env.quirks ? 370
        ...
        if ((CKEDITOR.env.ie7Compat || CKEDITOR.env.ie6Compat) && a.lang.dir == 'rtl') {
            this.parts
                .contents.setStyle('overflow', 'hidden');
        }
    }, onOk: function () {
        this.commitContent();
    }, contents: [
        {id: 'general', label: a.lang.common.generalTab, elements: [
            {type: 'html', id: 'securityMsg', html: '<div style="white-space:normal;width:340px;">'
                                                        + b.securityMsg + '</div>'},
            {type: 'html', id: 'pasteMsg', html: '<div style="white-space:normal;width:340px;">' + b
                .pasteMsg + '</div>'},
            {type: 'html', id: 'editing_area', style: 'width: 100%; height: 100%;', html: '', focus: function () {
                ...
                var e = this.getDialog(), f = '<html dir="' + a.config.contentsLangDirection + '"'
                                                  + ' lang="' + (a.config.contentsLanguage || a
                    .langCode) + '">'
                                                  + '<head><style>body { margin: 3px; height: 95%; } </style></head><body>'
                                                  + '<script id="cke_actscrpt" type="text/javascript">'
                                                  + 'window.parent.CKEDITOR.tools.callFunction( '
                                                  + CKEDITOR.tools.addFunction(d, e) + ', this );'
                                                  + '</script></body>' + '</html>', g = CKEDITOR.env
                                                                                            .air
                    ? 'javascript:void(0)' : c
                    ...
                    .createFromHtml('<iframe class="cke_pasteframe" frameborder="0"  allowTransparency="true" src="'
                                        + g + '"' + ' role="region"' + ' aria-label="' + b.pasteArea
                                        + '"' + ' aria-describedby="' + e
                ...
                var f = this.getElement(), g = this.getDialog().getParentEditor(), h = this
                    .getInputElement().getFrameDocument().getBody(), i = h.getBogus(), j;
                i && i.remove();
                j = h.getHtml();
                setTimeout(function () {
                    g.fire('paste', {html: j});
