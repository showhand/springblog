    ...
    function SearchFilter(jQ, fields, options) {
        ...
        if (fields != null) { // type coercion matches undefined as well as null
            ...
            function buildSel(className, options, isHidden) {
                return "<select class='" + className + "'" + (isHidden ? " style='display:none;'"
                    : "") + ">" + options + "</select>";
            }
            ...
            jQ.html("").addClass("ui-searchFilter")
                .append("<div class='ui-widget-overlay' style='z-index: -1'>&#160;</div><table class='ui-widget-content ui-corner-all'><thead><tr><td colspan='5' class='ui-widget-header ui-corner-all' style='line-height: 18px;'><div class='ui-closer ui-state-default ui-corner-all ui-helper-clearfix' style='float: right;'><span class='ui-icon ui-icon-close'></span></div>"
                            ...
                            + opts.searchText + "</span></span><span class='matchText'>" + opts
                            .matchText + "</span> " + gOps_html + " <span class='rulesText'>" + opts
                            .rulesText
            ...
            if (has_custom_ops) {
                jFSelect.change(function (e) {
                    ...
                    if (jElem[0] == null) {
                        jElem = td.find(".default");
                    } // if there's not an element for that field, use the default one
            ...
            jQ.find(".ui-del").click(function (e) {
                ...
                if (row.siblings(".sf").length > 0) { // doesn't remove if there's only one filter left
                    if (opts.datepickerFix === true && jQuery.fn.datepicker !== undefined) {
                        row.find(".hasDatepicker").datepicker("destroy");
                    } // clean up datepicker's $.data mess
            ...
            jQ.find(".ui-add").click(function (e) {
                ...
                if (opts.datepickerFix === true && jQuery.fn.datepicker !== undefined) { // using $.data to associate data with document elements is Not Good
                    row.find(".hasDatepicker").each(function () {
                        ...
                        newRow.find("#" + this.id).unbind().removeAttr("id")
                            .removeClass("hasDatepicker").datepicker(settings);
                    });
                }
